(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(erc-autojoin-mode t)
 '(erc-button-mode t)
 '(erc-fill-mode t)
 '(erc-header-line-uses-tabbar-p t)
 '(erc-highlight-nicknames-mode t)
 '(erc-input-line-position -1)
 '(erc-irccontrols-mode t)
 '(erc-list-mode t)
 '(erc-match-mode t)
 '(erc-menu-mode t)
 '(erc-move-to-prompt-mode t)
 '(erc-netsplit-mode t)
 '(erc-networks-mode t)
 '(erc-noncommands-mode t)
 '(erc-pcomplete-mode t)
 '(erc-prompt ">")
 '(erc-readonly-mode t)
 '(erc-ring-mode t)
 '(erc-scrolltobottom-mode nil)
 '(erc-speedbar-sort-users-type (quote alphabetical))
 '(erc-stamp-mode t)
 '(erc-track-minor-mode t)
 '(erc-track-mode t)
 '(ibuffer-default-sorting-mode (quote filename/process))
 '(ibuffer-display-summary nil)
 '(ibuffer-formats (quote ((modified " " name))))
 '(ibuffer-mode-hook (quote (ibuffer-auto-mode)))
 '(ibuffer-read-only-char 32)
 '(ibuffer-title-face (quote font-lock-type-face))
 '(ibuffer-truncate-lines t)
 '(ibuffer-use-header-line nil)
 '(ibuffer-use-other-window nil)
 '(indent-tabs-mode nil)
 '(js2-strict-trailing-comma-warning nil)
 '(pop-up-windows t)
 '(quote (erc-fill-mode t))
 '(scroll-bar-mode nil)
 '(scroll-step 1)
 '(smooth-scroll-mode t)
 '(smooth-scroll/hscroll-step-size 1)
 '(smooth-scroll/vscroll-step-size 1)
 '(speedbar-default-position (quote left))
 '(tabbar-auto-scroll-flag t)
 '(tabbar-background-color nil)
 '(tabbar-buffer-home-button
   (quote
    ((" + "
      (:type pbm :data "P2 13 13 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 0
6 0 255 255 255 255 255 255 255 255 255 255 9 130 9 255 255 255 255
255 255 255 255 255 255 26 130 26 255 255 255 255 255 255 255 0 9 26
41 130 41 26 9 0 255 255 255 255 5 145 140 135 130 125 120 115 5 255
255 255 255 0 9 26 41 130 41 26 9 0 255 255 255 255 255 255 255 26 130
26 255 255 255 255 255 255 255 255 255 255 9 130 9 255 255 255 255 255
255 255 255 255 255 0 6 0 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255
" :ascent center :mask
(heuristic t)
:margin 1))
     "[-]"
     (:type pbm :data "P2 13 13 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 0 0 1 2 3 2 1 0 0 255 255 255 255 0 132 128 123 119 114 110
106 0 255 255 255 255 0 0 1 2 3 2 1 0 0 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 255
" :ascent center :mask
(heuristic t)
:margin 1))))
 '(tabbar-cycle-scope (quote tabs))
 '(tabbar-home-button (quote (("[o]") "[x]")))
 '(tabbar-mode t nil (tabbar))
 '(tabbar-mwheel-mode nil nil (tabbar))
'(tabbar-scroll-left-button
(quote
 ((" < "
   (:type pbm :data "P2 13 13 255 255 255 255 255 255 255 255 255 255 255 255 255 255 255
255 255 255 255 255 255 255 255 128 16 48 255 255 255 255 255 255 255
255 144 28 86 128 0 255 255 255 255 255 255 160 44 92 159 135 113 0
255 255 255 255 160 44 97 165 144 129 120 117 0 255 255 176 44 98 175
174 146 127 126 127 128 0 255 255 0 160 184 156 143 136 134 135 137
138 0 255 255 176 32 67 144 146 144 145 146 148 149 0 255 255 255 255
160 42 75 140 154 158 159 160 0 255 255 255 255 255 255 160 40 74 154
170 171 0 255 255 255 255 255 255 255 255 160 41 82 163 0 255 255 255
255 255 255 255 255 255 255 160 32 48 255 255 255 255 255 255 255 255
255 255 255 255 255 255
" :ascent center :mask
(heuristic t)
:margin 1))
  " = ")))
 '(tabbar-scroll-right-button (quote ((" > ") " = ")))
 '(tabbar-separator (quote (0.3)))
 '(tabbar-use-images nil))

(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(speedbar-button-face ((t nil)))
 '(tabbar-button ((t (:inherit tabbar-default :height 1.3 :family "DejaVu Sans Mono"))))
 '(tabbar-button-highlight ((t nil)))
 '(tabbar-default ((t (:background "gray20" :foreground "grey75" :height 1.0))))
 '(tabbar-highlight ((t nil)))
 '(tabbar-modified ((t (:foreground "orange red"))))
 '(tabbar-selected ((t (:background "gray11" :foreground "gainsboro" :box (:line-width 2 :color "gray10")))))
 '(tabbar-selected-modified ((t (:inherit tabbar-modified :foreground "OrangeRed3"))))
 '(tabbar-separator ((t (:width normal))))
 '(tabbar-unselected ((t (:inherit tabbar-default :foreground "dark gray")))))
